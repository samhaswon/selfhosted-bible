...
class Navigate(FlaskForm):
    ...
    select_book = SelectField('Select book', choices=choices, validators=[DataRequired()])
...
class SelectChapter(FlaskForm):
    books = ESV().books
    choices: List[str] = [book.title for book in books]

    select = SelectField('Chapter', choices=choices, validators=[DataRequired()])
